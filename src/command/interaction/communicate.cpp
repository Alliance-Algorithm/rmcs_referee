...
public:
    Communicate()
        ...
        register_input("/referee/enemies/hero/position", enemies_hero_sentry_link_, false);
        register_input("/referee/enemies/engineer/position", enemies_engineer_sentry_link_, false);
        register_input("/referee/enemies/infantry_iii/position", enemies_infantry_iii_sentry_link_, false);
        register_input("/referee/enemies/infantry_iv/position", enemies_infantry_iv_sentry_link_, false);
        register_input("/referee/enemies/infantry_v/position", enemies_infantry_v_sentry_link_, false);
        register_input("/referee/enemies/sentry/position", enemies_sentry_sentry_link_, false);

        register_input("/referee/friends/hero/position", friends_hero_origin_link_, false);
        register_input("/referee/friends/engineer/position", friends_engineer_origin_link_, false);
        register_input("/referee/friends/infantry_iii/position", friends_infantry_iii_origin_link_, false);
        register_input("/referee/friends/infantry_iv/position", friends_infantry_iv_origin_link_, false);
        register_input("/referee/friends/infantry_v/position", friends_infantry_v_origin_link_, false);
        register_input("/referee/friends/sentry/position", friends_sentry_origin_link_, false);
        ...
        game_status_publisher_ =
            create_publisher<rmcs_msgs::msg::GameStatus>("/rmcs_referee/game_status", 10);
    ...
    void update() override {
        ...
        auto infantry_iii_origin_link_ = transform(*enemies_infantry_iii_sentry_link_);
        auto infantry_iv_origin_link_  = transform(*enemies_infantry_iv_sentry_link_);
        ...
        status.enemies_hero.pose.x         = hero_origin_link.x();
        status.enemies_hero.pose.y         = hero_origin_link.y();
        status.enemies_engineer.pose.x     = engineer_origin_link_.x();
        status.enemies_engineer.pose.y     = engineer_origin_link_.y();
        status.enemies_infantry_iii.pose.x = infantry_iii_origin_link_.x();
        status.enemies_infantry_iii.pose.y = infantry_iii_origin_link_.y();
        status.enemies_infantry_iv.pose.x  = infantry_iv_origin_link_.x();
        status.enemies_infantry_iv.pose.y  = infantry_iv_origin_link_.y();
        status.enemies_infantry_v.pose.x   = infantry_v_origin_link_.x();
        status.enemies_infantry_v.pose.y   = infantry_v_origin_link_.y();
        status.enemies_sentry.pose.x       = sentry_origin_link_.x();
        status.enemies_sentry.pose.y       = sentry_origin_link_.y();

        status.friends_hero.pose.x         = friends_hero_origin_link_->x();
        status.friends_hero.pose.y         = friends_hero_origin_link_->y();
        status.friends_engineer.pose.x     = friends_engineer_origin_link_->x();
        status.friends_engineer.pose.y     = friends_engineer_origin_link_->y();
        status.friends_infantry_iii.pose.x = friends_infantry_iii_origin_link_->x();
        status.friends_infantry_iii.pose.y = friends_infantry_iii_origin_link_->y();
        status.friends_infantry_iv.pose.x  = friends_infantry_iv_origin_link_->x();
        status.friends_infantry_iv.pose.y  = friends_infantry_iv_origin_link_->y();
        status.friends_infantry_v.pose.x   = friends_infantry_v_origin_link_->x();
        status.friends_infantry_v.pose.y   = friends_infantry_v_origin_link_->y();
        ...
        game_status_publisher_->publish(status);
...
private:
    ...
    std::shared_ptr<rclcpp::Publisher<rmcs_msgs::msg::GameStatus>> game_status_publisher_;
