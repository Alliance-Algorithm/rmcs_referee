...
public:
  Status()
    ...
    try {
      register_output("/referee/serial", serial_,
                      get_parameter("path").as_string(), 115200,
                      serial::Timeout::simpleTimeout(0));
    } catch (serial::IOException &ex) {
      RCLCPP_ERROR(logger_, "Unable to open serial port: %s", ex.what());
    }

    register_output("/referee/game/stage", game_stage_,
                    rmcs_msgs::GameStage::UNKNOWN);
    ...
    register_output("/referee/shooter/heat_limit", robot_shooter_heat_limit_,
                    0);
    register_output("/referee/chassis/power_limit", robot_chassis_power_limit_,
                    0.0);
    register_output("/referee/chassis/power", robot_chassis_power_, 0.0);
    register_output("/referee/chassis/buffer_energy", robot_buffer_energy_,
                    60.0);

    register_output("/referee/friends/hero/position", pose_hero_);
    register_output("/referee/friends/engineer/position", pose_engineer_);
    register_output("/referee/friends/infantry_iii/position",
                    pose_infantry_iii_);
    ...
    register_output("/referee/shooter/bullet_allowance",
                    robot_bullet_allowance_, false);
    position_pub_ = create_publisher<std_msgs::msg::Float32MultiArray>(
  ...
  void update() override {
    ...
    if (cache_size_ >= sizeof(frame_.header)) {
      ...
      cache_size_ +=
          serial_->read(reinterpret_cast<uint8_t *>(&frame_) + cache_size_,
    ...
    if (game_status_watchdog_.tick()) {
      RCLCPP_INFO(logger_,
                  "Game status receiving timeout. Set stage to unknown.");
      *game_stage_ = rmcs_msgs::GameStage::UNKNOWN;
    }
    if (robot_status_watchdog_.tick()) {
      RCLCPP_ERROR(logger_,
                   "Robot status receiving timeout. Set to safe indicators.");
    ...
    if (power_heat_data_watchdog_.tick()) {
      RCLCPP_ERROR(logger_,
                   "Power heat data receiving timeout. Set to initial values.");
